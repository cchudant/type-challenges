...
type Enum2<T extends readonly string[], N extends boolean = false, Inc extends any[] = []> =
    T extends readonly [infer Fst, ...infer Rest]
        ? (Fst extends string ? Rest extends readonly string[]
            ? { readonly [K in Capitalize<Fst>]: N extends true ? Inc['length'] : Fst }
                & Enum2<Rest, N, [...Inc, any]>
            : never : never)
        : {}
