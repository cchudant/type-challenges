...
type DistributeObjectUnion<T, Keys extends keyof T = keyof T, Values = DistributeUnions<T[keyof T]>> =
    [Keys] extends [never] ? {} :
        Values extends Values ? Keys extends Keys
            ? (Values extends T[Keys]
                ? { [_ in Keys]: Values } & DistributeObjectUnion<Omit<T, Keys>>
                : never)
            : {} : {}
type DistributeTupleUnion<T> = T extends [infer Fst, ...infer Rest]
        ? (DistributeUnions<Fst> extends infer FstU
            ? (FstU extends FstU ? [FstU, ...AssertArray<DistributeTupleUnion<Rest>>] : never)
            : never)
    : []
type DistributeUnions<T> =
    T extends [...any[]] ? DistributeTupleUnion<T>
    : T extends object ? Simplify<DistributeObjectUnion<T>>
    : T
