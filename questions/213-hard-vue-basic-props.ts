...
type ComputedProperties<Computed> =  { [X in keyof Computed]: Computed[X] extends (...args: any[]) => infer R ? R : never }
type GetPropType<T> = T extends { type: infer R } ? GetPropType2<R> : GetPropType2<T>
type GetPropType2<T> =
    T extends (...arg: any[]) => infer R ? R
    : T extends new (...arg: any[]) => infer R ? R
    : T extends { [key: string]: never } ? any
    : T extends (infer R)[] ? GetPropType<R>
    : never
type PropsProperties<Props> = { [K in keyof Props]: GetPropType<Props[K]> }
type Options<Data, Computed, Methods, Props> = {
    props?: Props,
    data?: (this: PropsProperties<Props>) => Data,
    methods?: Methods & ThisType<Data & Methods & ComputedProperties<Computed> & PropsProperties<Props>>,
    computed?: Computed & ThisType<Data & Methods & ComputedProperties<Computed> & PropsProperties<Props>>,
}
declare function VueBasicProps<Data, Computed, Methods, Props>(options: Options<Data, Computed, Methods, Props>):
