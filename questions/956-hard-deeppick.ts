...
type DeepPick2<T, K extends string> =
    K extends `${infer Fst}.${infer Rest}`
        ? (Fst extends keyof T
            ? { [K2 in Fst]: DeepPick2<T[Fst], Rest> }
            : never)
        : { [K2 in K]: (K extends keyof T ? T[K] : never) }
type DeepPick<T, K extends string> = UnionToIntersection<K extends '' ? unknown : DeepPick2<T, K>>


/* _____________ Test Cases _____________ */
import { Equal, Expect, ExpectFalse, NotEqual, UnionToIntersection } from '@type-challenges/utils'
