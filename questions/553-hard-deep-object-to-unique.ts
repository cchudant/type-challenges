...
const brandSym = Symbol()
type Branded<T, Brand> = T & { readonly [brandSym]?: Brand }
type DeepObjectToUniq<O extends any, Path extends any[] = [], Root = O> =
    O extends object
        ? Branded<{
            [K in keyof O]: DeepObjectToUniq<O[K], [...Path, K], Root>
        }, [Root, ...Path]>
        : O
