...
type IsUnion<T, C extends T = T> =
    (T extends T ? C extends T ? true : unknown : never) extends true
        ? false
        : true
type TplNFirstElements<Tpl extends readonly any[], NArr extends readonly any[], Ret extends readonly any[] = []> =
    NArr extends [] ? Ret :
        (NArr extends [infer _, ...infer Rest]
        ? Tpl extends [infer TplFst, ...infer TplRest]
        ? TplNFirstElements<TplRest, Rest, [...Ret, TplFst]>
        : never : never)

type IsNever<T> = [T] extends [never] ? true : false
type IsUnknown<T> = 0 extends (1 & T) ? (IsAny<T> extends true ? false : true) : false
type RemoveObjectFromUnion<U> =
    U extends { [x: string]: never }
        ? U
        : (IsNever<keyof U> extends false ? U : never)

type GetTagObj2<B> =
    B extends { readonly [UNIQUE_SYMBOL]?: infer TagObj }
        ? RemoveObjectFromUnion<TagObj>
        : []
type GetTagObj<B> = IsNever<GetTagObj2<B>> extends true ? [] : GetTagObj2<B>
type SetTagObj<B, O> = RemoveTagObj<B> & { readonly [UNIQUE_SYMBOL]?: O | object }
type RemoveTagObj<B> = IsUnknown<B> extends true ? B : { [K in keyof B as K extends US ? never : K]: B[K] }

type RemoveNullUndefinedFromUnion<B> =
    B extends null ? never
    : B extends undefined ? never
    : B
type GetTags<B> = IsNever<RemoveNullUndefinedFromUnion<B>> extends true
    ? []
    : (IsUnion<GetTagObj<RemoveNullUndefinedFromUnion<B>>> extends true
        ? []
        : GetTagObj<RemoveNullUndefinedFromUnion<B>>)

type Tag<B, T extends string> =
    IsNever<B> extends true ? Tag<unknown, T>
    : IsAny<B> extends true ? Tag<unknown, T>
    : B extends null ? B
    : B extends undefined ? B
    : SetTagObj<B, [...GetTagObj<B>, T]>

type UnTag<B> = RemoveTagObj<B>

type HasTagFallbackAny<B, T extends string> =
    IsNever<B> extends true
    ? any
    : (B extends B
        ? (IsNever<GetTagObj<B>[number] & T> extends false ? keyof GetTagObj<B> : any)
        : never)
type HasTag<B, T extends string> = IsAny<HasTagFallbackAny<RemoveNullUndefinedFromUnion<B>, T>> extends true
    ? false : true
type HasTags<B, T extends readonly string[], Tags = GetTags<B>> =
    Tags extends [infer Fst, ...infer Rest]
        ? (Equal<TplNFirstElements<[Fst, ...Rest], T>, T> extends true
            ? true
            : HasTags<B, T, Rest>)
        : Equal<T, []>
...
import { IsTrue, Expect, Equal, IsAny } from "@type-challenges/utils";
